...
import {
  ...
  ERROR_DISPLAY_TIME_SECONDS,
  MILLISECONDS_IN_SECONDS,
} from "@/constants"
import { type ApiResponse } from "@/types/ApiResponse"

const handleChangeOnInput =
  <T,>(fn: (newValue: T) => void, value: keyof HTMLInputElement = "value") =>
...
const NumberSelector = () => {
  ...
  const handleChangeShowExtendedForm = handleChangeOnInput<boolean>(
    setShowExtendedForm,
    "checked",
  )
    ...
    (async () => {
      try {
        ...
        if (!success) {
          ...
          }, ERROR_DISPLAY_TIME_SECONDS * MILLISECONDS_IN_SECONDS)
        }
      ...
      <div className="py-5 mx-16 text-slate-600 text-lg">
        ...
        <div className="flex items-center gap-3">
          <p className="text-slate-600 font-semibold text-lg">
            I want multiples number
          </p>
          <input
            type="checkbox"
            className="bg-slate-200"
            onChange={handleChangeShowExtendedForm}
          />
          {showExtendedForm && (
            <NumberInput
              onChange={handleChangeResultLength}
              value={resultLength}
            />
          )}
        </div>
