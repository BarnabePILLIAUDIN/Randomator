...
const corsOrigin =
  process.env.CORS ??
  (() => {
    throw new Error(MISSING_CORS_ERROR)
  })()
...
app.get(`${RANDOM_ROUTES}/:length`, (ctx) => {
  const inputLength = Number.parseInt(ctx.req.param("length"), RADIX)
  const { min, max } = extractMinAndMax(ctx)
  const numbers = Array.from(
    new Array(!Number.isNaN(inputLength) ? inputLength : DEFAULT_LENGTH),
